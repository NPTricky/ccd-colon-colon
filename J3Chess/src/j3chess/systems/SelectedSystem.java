...
public class SelectedSystem extends EntityProcessingSystem {
    ...
    public SelectedSystem(final J3ChessView view) {
    ...
    protected void process(final Entity e) {
        ...
        final Position position = mPositionMapper.get(e);

        final Graphics2D graphics = J3ChessApp.getInstance().getDrawGraphics();

        final Vector2d drawPosition = position.getField().getDrawPosition(
                mDrawPanelSize.x, mDrawPanelSize.y);

        // Draw selection ring
        graphics.drawImage(
                SELECTION_OVERLAY.getImage(),
                Math.round(drawPosition.x - SELECTION_OVERLAY.getIconWidth()
                        / 2.0f + mDrawOffset.x),
                Math.round(drawPosition.y - SELECTION_OVERLAY.getIconHeight()
                        / 2.0f + mDrawOffset.y), null);

        // Draw valid moves
        final ValidMovement moves = (ValidMovement) e
        ...
        if (moves != null) {
            for (final Move move : moves.getValidNonCaptureMoves()) {
                ...
                graphics.drawImage(
                        ...
                        Math.round(fDrawPosition.x
                                - MOVE_OVERLAY.getIconWidth() / 2.0f
                                + mDrawOffset.x),
                        Math.round(fDrawPosition.y
                                - MOVE_OVERLAY.getIconHeight() / 2.0f
                                + mDrawOffset.y), null);
            }
