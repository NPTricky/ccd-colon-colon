...
public class Move {
    ...
    /**
     * @brief complete constructor for a move crossedCenter defaults to false
     * @param type
     *            type of the move
     * @param start
     *            start of the move
     ...
     *            target of the move
     */
    public Move(final MoveType type, final Field start, final Field target) {
     ...
     *            target of the move
     * @param crossedCenter
     *            whether the move crossed the center
     */
    public Move(final MoveType type, final Field start, final Field target,
    ...
    final void generateDescription() {
        // pieces
        final ComponentType contextType = ComponentType
                .getTypeFor(PieceContext.class);
        final String movingPiece = ((PieceContext) mMovingPiece
                .getComponent(contextType)).getPieceType().toAlgebraic();
        String targetPiece = "";
        if (mTargetPiece != null) {
            targetPiece = ((PieceContext) mTargetPiece
                    .getComponent(contextType)).getPieceType().toAlgebraic();
        }
     ...
     * 
     *        Common Move: <PieceType><mTarget> mType.toAlgebraic() +
     *        mTarget.toString() Disambiguating Move:
     *        <PieceType><mStart.File><mStart.Rank>... mType.toAlgebraic() +
     *        mStart.toString() ...
     * 
     *        Capture: <PieceType>x<mTarget> mType.toAlgebraic() + "x" +
     *        mTarget.toString() Capture En Passant: <mStart.File>x<mTarget>e.p.
     *        mStart.toFile() + "x" + mTarget.toString() + "e.p."
     * 
     *        Promotion: <mTarget><PieceType> mTarget.toString() +
     *        mPromotionType.toAlgebraic()
     * 
     *        [TRIVIAL] Kingside Castling: 0-0 Queenside Castling: 0-0-0
     * 
     *        [SUFFIX] Check: <CompleteMove>+ Checkmate: <CompleteMove>#
     * 
    ...
    public final void execute() {
        final ComponentType positionType = ComponentType
                .getTypeFor(Position.class);
        final ComponentType movementType = ComponentType
                .getTypeFor(Movement.class);
        // notify moving piece
        ((Position) mMovingPiece.getComponent(positionType))
                .setField(mTargetField);
        if (this.mIsCrossingCenter) {
            ((Movement) mMovingPiece.getComponent(movementType))
                    .toggleCrossedCenter();
        }
