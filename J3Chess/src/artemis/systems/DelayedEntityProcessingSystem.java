 ...
 * 
 * Implementation notes: In order to start the system you need to override the
 * inserted(Entity e) method, look up the delay time from that entity and offer
 * it to the system by using the offerDelay(float delay) method. Also, when
 * processing the entities you must also call offerDelay(float delay) for all
 * valid entities.
 * 
 * @author Arni Arent
 * 
...
public abstract class DelayedEntityProcessingSystem extends EntitySystem {
     ...
     * is shorter than the time remaining, the system will restart itself to run
     * at the offered delay.
     * 
     * @param delay
     */
    ...
    /**
     * Get the time until the system is scheduled to run at. Returns zero (0) if
     * the system is not running. Use isRunning() before checking this value.
     * 
    ...
    /**
     * Stops the system from running, aborts current countdown. Call offerDelay
     * or restart to run it again.
     */
    public void stop() {
        this.running = false;
        this.acc = 0;
