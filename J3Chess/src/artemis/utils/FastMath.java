...
public class FastMath {
    ...
    public final static double asin(final double x) {
        return x * (Math.abs(x) * (Math.abs(x) * _asin_a + _asin_b) + _asin_c)
                + Math.signum(x) * (_asin_d - Math.sqrt(1 - x * x));
    }
    ...
    public final static double atan(final double x) {
        return (Math.abs(x) < 1) ? x / (1 + _atan_a * x * x) : Math.signum(x)
                * HALF_PI - x / (x * x + _atan_a);
    }

    public final static double inverseSqrt(double x) {
        final double xhalves = 0.5d * x;
        x = Double.longBitsToDouble(0x5FE6EB50C7B537AAl - (Double
